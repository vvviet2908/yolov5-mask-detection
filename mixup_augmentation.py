    ...
    for index in tqdm(range(NUMBER_IMAGES)):
        ...
        if max(bbox_list) < 0.05:
            ...
            file_name = img_paths[idxs[0]].split(
                '/')[-1].rsplit('.', 1)[0]
            cv2.imwrite("imgs_augment/auto_augment/images/{}_{}_{}.jpg".format(file_name, TYPE_TRANSFORM,
                        letter_code), img_mixup, [cv2.IMWRITE_JPEG_QUALITY, 80])
            with open("imgs_augment/auto_augment/labels/{}_{}_{}.txt".format(file_name, TYPE_TRANSFORM, letter_code), "w") as outfile:
